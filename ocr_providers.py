...
class PaddleOCRProvider(OCRProvider):
    ...
    Uses PaddleOCR v3.x with PP-OCRv5 models for high-accuracy
...
class DeepSeekOCRProvider(OCRProvider):
    ...
    def _initialize_transformers(self) -> None:
        ...
        try:
            ...
            logger.info(f"Loading DeepSeek-OCR model: {self.config.model_name}")
        ...
        **kwargs
        ...
        if provider_type == OCRProviderType.PADDLEOCR:
            return PaddleOCRConfig(
                ...
                rec_thresh=kwargs.get('rec_thresh', 0.3),
            )
        ...
        **kwargs
        ...
        for provider in self.providers:
            if not provider.is_available:
                continue
            try:
                result = provider.recognize(image, **kwargs)
                results.append(result)
            except Exception as e:
