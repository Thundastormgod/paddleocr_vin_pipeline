...
- Support for full fine-tuning on larger GPUs (48GB+)
- Mixed precision training (bf16/fp16) - RTX 3090 supports both
- Gradient checkpointing for memory efficiency
- DeepSpeed integration for distributed training
- Hugging Face Trainer with custom metrics

RTX 3090 (24GB) Recommended Settings:
- LoRA with bf16: batch_size=4, gradient_accumulation=8
...
Usage (on HPC with RTX 3090):
    # LoRA fine-tuning (recommended for RTX 3090)
    ...
    # With 8-bit quantization (if OOM)
    python -m src.vin_ocr.training.finetune_deepseek \\
        --config configs/deepseek_finetune_config.yml \\
        --lora \\
        --load-in-8bit
...
try:
    ...
    # Define fallback types for type checking only - actual training requires torch
    TORCH_AVAILABLE = False
    TorchDataset = object
    DataLoader = None
...
try:
    ...
    PEFT_AVAILABLE = False  # Availability checked at runtime
...
class DeepSeekFineTuneConfig:
    ...
    Optimized defaults for NVIDIA RTX 3090 (24GB VRAM).
    """
    # Model
    ...
    num_epochs: int = 10
    batch_size: int = 4  # Safe for 24GB with LoRA
    ...
    # Memory optimization (important for RTX 3090)
    gradient_checkpointing: bool = True  # Reduces memory ~20% slower
    ...
    # Quantization (use if OOM with bf16)
    use_8bit: bool = False
    use_4bit: bool = False
    ...
    # Precision (RTX 3090 supports both)
    bf16: bool = True   # Recommended for RTX 3090 (Ampere)
    fp16: bool = False
...
class VINDeepSeekDataset(TorchDataset):
    ...
    def __init__(
        ...
        # Some models return a processor with .tokenizer, others return tokenizer directly
...
class DeepSeekVINTrainer:
    ...
    def setup_model(self):
        ...
        # Use hardware detection for optimal configuration
        if HARDWARE_UTILS_AVAILABLE:
            hardware_detector = HardwareDetector()
            hw_config = hardware_detector.get_training_config("deepseek")
            ...
            logger.info(f"  CUDA available: {hw_config.get('use_cuda', False)}")
            logger.info(f"  MPS available: {hw_config.get('use_mps', False)}")
            logger.info(f"  4-bit quantization: {hw_config.get('use_4bit', False)}")
            logger.info(f"  8-bit quantization: {hw_config.get('use_8bit', False)}")
            logger.info(f"  LoRA recommended: {hw_config.get('use_lora', True)}")
            logger.info(f"  Recommended batch size: {hw_config.get('recommended_batch_size', 1)}")
            
            device = hw_config.get('device', 'cpu')
            
            # Override config based on hardware capabilities
            if not hw_config.get('use_4bit', False):
                if self.config.use_4bit:
                    logger.warning("4-bit quantization not supported on this hardware. Disabling.")
                    self.config.use_4bit = False
            if not hw_config.get('use_8bit', False):
                if self.config.use_8bit:
                    logger.warning("8-bit quantization not supported on this hardware. Disabling.")
                    self.config.use_8bit = False
        else:
            # Fallback to manual device detection
            device = self._get_device()
        
        logger.info(f"Using device: {device}")
        
        # Quantization config - only available with CUDA
        quantization_config = None
        if (self.config.use_4bit or self.config.use_8bit) and device != "cuda":
            logger.warning("Quantization (4-bit/8-bit) requires CUDA. Disabling quantization.")
            self.config.use_4bit = False
            self.config.use_8bit = False
        ...
        # Load model
        # Use float32 for MPS (bfloat16 not fully supported)
        if device == "mps":
            dtype = torch.float32
            logger.info("Using float32 for MPS device (bfloat16 not fully supported)")
        else:
        ...
        # Note: For some models, AutoProcessor returns the tokenizer directly
        # For others, it returns a processor with a .tokenizer attribute
        self.processor = AutoProcessor.from_pretrained(
            self.config.model_name,
            trust_remote_code=True
        )
        
        # Handle both cases: processor with tokenizer attribute, or tokenizer directly
        if hasattr(self.processor, 'tokenizer'):
            self.tokenizer = self.processor.tokenizer
        else:
            # The processor IS the tokenizer
